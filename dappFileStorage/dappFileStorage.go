 ...
 */
func (s *SmartContract) UpdateFileProgress(ctx contractapi.TransactionContextInterface, id, provider string, state int) error {
	var err error
	if exists, err := s.FileExists(ctx, id); err != nil {
		return err
	} else if exists {
		...
		return ctx.GetStub().PutState(id, updatedFile)
	}
